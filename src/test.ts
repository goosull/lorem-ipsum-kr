import loremIpsumKR from "./index";

// 기본 1 문장 생성 테스트
const output1 = loremIpsumKR();
...
if (sentences1.length === 1) {
  ...
  console.error(
    `Test 1: Fail (Expected 1 sentence, but got ${sentences1.length}): ${output1}`
  );
}

// 문장 단어 수 제한 테스트
const output2 = loremIpsumKR({
  count: 1,
  units: "sentences",
...
if (words2.length >= 5 && words2.length <= 7) {
  ...
  console.error(
    `Test 2: Fail (Expected 5~7 words in a sentence, but got ${words2.length}): ${output2}`
  );
}

// 2개의 문단 생성 테스트 (각 문단의 문장 수는 3~5 사이)
const output3 = loremIpsumKR({
  count: 2,
  units: "paragraphs",
  sentenceLowerBound: 5,
  sentenceUpperBound: 7,
...
if (paragraphs3.length === 2) {
  ...
  console.error(
    `Test 3: Fail (Expected 2 paragraphs, but got ${paragraphs3.length}): ${output3}`
  );
}
...
paragraphs3.forEach((paragraph, index) => {
  ...
  if (sentenceCount >= 3 && sentenceCount <= 5) {
    console.log(
      `Test 3-${index + 1}: Pass (문단 ${index + 1}: ${sentenceCount}개의 문장)`
    );
  } else {
    console.error(
      `Test 3-${index + 1}: Fail (문단 ${
        index + 1
      }에 ${sentenceCount}개의 문장이 있습니다. 예상 범위는 3 ~ 5입니다.): ${paragraph}`
    );
  }
});

// HTML 형식 출력 테스트
const output4 = loremIpsumKR({
  count: 1,
  units: "paragraphs",
  format: "html",
});
...
try {
  ...
  console.error(
    `Test 5: Fail (Expected an error for invalid sentence bounds) ${output5}`
  );
} catch (error) {
...
if (fixedWords.length === 10) {
  ...
  console.error(
    `Test 6: Fail (Expected 10 words, but got ${fixedWords.length})`
  );
}

// 단어 단위 생성 테스트
const output7 = loremIpsumKR({
  count: 5,
  units: "words",
});
...
if (words7.length === 5) {
  ...
  console.error(
    `Test 7: Fail (Expected 5 words, but got ${words7.length}): ${output7}`
  );
}
